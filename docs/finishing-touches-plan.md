...
- [x] **split Pricing into 3 page views** `M` — route-based per-tier pages
      (`/pricing/:tier`), nav links + prev/next + swipe. Committed.
- [ ] **notification opt-in in onboarding** `M` — the notifications toggle
      ...
      during sign-on. Move the ask into onboarding — a real prompt for
      whether someone wants notifications, stating the cadence (broadcasts go
      out Thursdays, Pacific time) so it's not a surprise later. Web-only
      instances without `realness-functions` deployed should skip this
      (`push` capability off).

### 📜 Licensing

- [x] **switch to GPL-2.0** `M` — replaced the source-available license, so
      closed forks aren't possible; self-hosting stays granted. `src/potrace/`
      now credits potrace/node-potrace. Committed.
...
- [x] **public-facing changelog** `M` — Changelog is the last section of
      `/docs` (raw `CHANGELOG.md`, dates humanized); `/changelog` redirects
      there. Committed. Still open: curated prose vs. raw log; push
      notifications v2 tie-in.
