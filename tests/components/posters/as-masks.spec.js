...
describe('@/components/posters/as-masks.vue', () => {
  it('renders horizontal, radial, and vertical masks with suffixed ids', () => {
    ...
    expect(masks[3].attributes('id')).toContain('cutout-shadow-dim')
  })
  ...
  it('emits one mask per subject from the live subject list', () => {
    ...
    const masks = wrapper.findAll('mask')
    // 4 base masks + 1 per subject
    expect(masks).toHaveLength(6)
    expect(masks[4].attributes('id')).toContain(`subjects/1700000900001`)
    expect(masks[5].attributes('id')).toContain(`subjects/1700000900002`)
  })
})
